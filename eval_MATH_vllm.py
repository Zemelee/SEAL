...
def main(args):
    ...
    for i, example in enumerate(test_data):
        ...
        if args.use_chat_format:  # 对话格式处理
            ...
            if (
                "gemma" in args.model_name_or_path
                or "deepseek" in args.model_name_or_path
            ):
                messages = [
                    {
                        "role": "user",
    ...
    if not args.logit_adjustment:
        ...
        logit_adjustment_tokens = torch.LongTensor(
            [
                ...
                if any([x in token for x in args.logit_adjustment_tokens])
            ]
        ).to("cuda")
    ...
    if args.peft is not None:
        outputs = model.generate(
            ...
            lora_request=LoRARequest("lora_path", 1, lora_path=args.peft),
        )
    else:
...
if __name__ == "__main__":
    ...
    if args.logit_adjustment:
        name = (
            "_".join(args.logit_adjustment_tokens)
            + f"_value_{args.logit_adjustment_value}"
        )
    ...
    eval_main(
        os.path.join(args.save_dir, "predictions.jsonl"),
        save=True,
        k=None,
